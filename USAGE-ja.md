...
12. [保存コンフィグ](#12-保存コンフィグ)
13. [ラン比較](#13-ラン比較)
14. [ドキュメント閲覧](#14-ドキュメント閲覧)
15. [mikon.toml リファレンス](#15-mikontoml-リファレンス)
16. [ストアのレイアウト](#16-ストアのレイアウト)
17. [GPU管理](#17-gpu-管理)
18. [認証・アクセス制御](#18-認証アクセス制御)
19. [API エラーモデル](#19-api-エラーモデル)
...
| パラメータ | 値 | デフォルト | 説明 |
| --- | --- | --- | --- |
| `direction` | `ancestors` / `descendants` / `both` | `both` | 探索方向 |
| `depth` | 0–20 | `2` | 何ホップまで辿るか |
| `include_modules` | `true` / `false` | `false` | `uses-module` エッジを含めるか |

**手動リンク：**

自動エッジが記録されない関係（「このランを参考にした」など）を手動でリンクできます。リンクはリネージュビューに表示されます。

```
POST /api/links
Body: { "src": "<node_id>", "dst": "<node_id>", "note": "<任意のテキスト>" }
...
| プレフィックス | 例 | 参照先 |
| --- | --- | --- |
| `run:` | `run:train__20260612-153000__a1b2` | ラン |
| `dataset:` | `dataset:imagenet` | 登録済みデータセット |
| `artifact:` | `artifact:train__20260612-153000__a1b2/weights/final.pt` | 成果物 |
| `module:` | `module:ResNet` | 登録済みモジュール |

---

## 11. 整理機能（タグ・グループ・スター・削除）

各ランにはアノテーションを付けられます。**起動時**（UI のジョブ起動フォームまたは `POST /api/runs`）に設定することも、**起動後**（UI の Overview タブまたは `PATCH /api/runs/{run_id}`）に編集することもできます。

**`POST /api/runs` リクエストボディ（`CreateRunRequest`）：**

| フィールド | 型 | デフォルト | 説明 |
| --- | --- | --- | --- |
| `job` | `str` | （必須） | ジョブ名 |
| `config` | `dict` | （必須） | コンフィグ値 |
| `gpus` | `list[str]` | （必須、1件以上） | GPU ID |
| `force` | `bool` | `false` | GPU占有中でも強制起動 |
| `annotations` | `Annotations \| null` | `null` | タグ・スター・グループ・タイトル・メモ |
| `save_config_as` | `str \| null` | `null` | 起動時にコンフィグをこの名前で保存（→§12参照） |
...
**保存コンフィグ**とは、ジョブのコンフィグ値に名前を付けて保存し、後から再利用できる仕組みです。UIのジョブ起動フォームで保存コンフィグを選択すると、フィールドが自動的に埋まります。

### コンフィグの保存

**起動時（UI）：** ジョブ起動フォームの「Save config as」欄に名前を入力すると、起動と同時に保存されます。

**起動時（API）：** `CreateRunRequest` の `save_config_as` フィールドに名前を指定します：
...
Body: { "job": "<ジョブ名>", "values": { ... } }
```

名前は `[A-Za-z0-9_.-]+` のみ使用可能です。既存の名前に上書き保存できます（同じジョブに限る。別ジョブの名前を使うと `409 config-name-conflict`）。

### 保存コンフィグの管理

```
GET    /api/configs               # 一覧取得
GET    /api/configs/{name}        # 個別取得
DELETE /api/configs/{name}        # 削除（204 No Content）
```

### コンフィグ差分（スキーマ変更の検出）

ジョブの Config クラスを変更した（フィールドの追加・削除・型変更）後に、保存済みコンフィグが現在のスキーマと互換性があるか確認できます：
...
| `name` | `str` | コンフィグ名 |
| `job` | `str` | 対象ジョブ |
| `compatible` | `bool` | 現スキーマと互換性があるか |
| `changes` | `list` | 差分一覧。各要素に `field`, `kind`（`added` / `removed` / `type_changed` / `constraint_changed`）, `detail` |
| `migrated_values` | `dict` | デフォルト補完後の値 |

`type_changed` または `constraint_changed` が1件でもあれば `compatible` は `false` になります。`added` / `removed` のみであれば互換性ありとみなされます（追加フィールドにはデフォルト値が補完され、削除フィールドは無視されます）。

---

## 13. ラン比較

複数のランのコンフィグとメトリクスを並べて比較します：
...
**レスポンス（`CompareRunsResponse`）：**

| フィールド | 説明 |
| --- | --- |
| `runs[]` | 各ランの `run_id`・`job`・`status`・`config`・メトリクス統計（`count` / `latest` / `min` / `max`） |
| `config_fields[]` | 全ランにわたる全コンフィグキーの一覧 |
| `metric_names[]` | 全ランにわたる全メトリクス名の一覧 |
| `config_diffs[]` | ランをまたいで値が異なるフィールドのみ。各要素に `field`, `values`（`run_id → 値` のマップ）, `missing_run_ids` |

`config_diffs` には値が異なるフィールドのみ含まれます。全ランで値が同一のフィールドは省略されます。
